...
Run a background shell command and get its output streamed back into the conversation as it happens, instead of blocking on it.
...
Call it with `action: "start"`, a `command`, and a `description`. The command keeps running after the tool call returns — its stdout is batched every 500ms and injected into the session as a new message (not as the tool's result), prefixed so the model knows it's an unprompted notification rather than user input. Use `action: "list"` to see running monitors, or `action: "stop"` with a `monitor_id` to end one early.

By default a monitor times out after `timeout_ms` (default 5 minutes, max 1 hour). Set `persistent: true` to keep it alive for the whole session instead, ignoring the timeout. Either way, a monitor that produces more than 20 output batches or 1MB of output within 60 seconds is killed automatically to prevent flooding the conversation.
