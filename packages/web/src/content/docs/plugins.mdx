...
Override how a permission request is resolved before opencode's own [permission](/docs/permissions) rules and [automode](/docs/permissions#automode) judge run:

```ts title=".opencode/plugins/permission-hook.ts"
import type { Plugin } from "@opencode-ai/plugin"

export const PermissionHookPlugin: Plugin = async (ctx) => {
  return {
    "permission.ask": async (input, output) => {
      if (input.permission === "bash" && input.patterns.includes("rm *")) {
        output.status = "deny"
      }
    },
  }
...
`output.status` accepts `"ask"`, `"deny"`, or `"allow"`. If the hook throws or doesn't set `output.status`, the request falls through to normal resolution (fails open).
