...
| `POST` | `/server/shutdown`  | Gracefully shut down a detached server: disposes all instances and stops the listener (auth required) | `204 No Content` |
| `POST` | `/server/handoff`   | Accept queued prompts from a detaching TUI client and replay them after the active turn finishes (auth required) | `204 No Content` |
...
| `PATCH`  | `/session/:id`                           | Update session properties             | body: `{ title?, automode? }`, returns <a href={typesUrl}><code>Session</code></a> |
