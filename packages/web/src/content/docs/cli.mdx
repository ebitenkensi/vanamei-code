...
| <nobr><code>{"--attach"}</code></nobr>       |       | Attach to a running opencode server instead of starting a local one (omit the URL to use the project's discovery record) |
| <nobr><code>{"--password"}</code></nobr>     | `-p`  | Basic auth password when attaching (defaults to `OPENCODE_SERVER_PASSWORD`)                                              |
| <nobr><code>{"--username"}</code></nobr>     | `-u`  | Basic auth username when attaching (defaults to `OPENCODE_SERVER_USERNAME` or `opencode`)                                |
| <nobr><code>{"--port"}</code></nobr>         |       | Port for the local server (defaults to a random port)                                                                    |
| <nobr><code>{"--no-replay"}</code></nobr>    |       | Disable session history replay on resume and after resize                                                                |
| <nobr><code>{"--replay-limit"}</code></nobr> |       | Cap visible replay to the newest N messages                                                                              |
| <nobr><code>{"--no-detach"}</code></nobr>     |       | Disable detachable-by-default mode; use legacy single-process local mode instead                                         |

This is the same interactive UI documented in full on the [TUI page](/docs/tui). Passing `--attach` here is equivalent to running [`opencode attach`](#attach). By default, `opencode` starts in detachable mode (spawns a child server and connects the TUI as an HTTP client); pass `--no-detach` or set `detach.enabled: false` in config to use the legacy single-process mode where `/detach` spawns a background server at detach time instead.
...
Attach a terminal to an already running OpenCode backend server started via `serve` or `web` commands, or to a server backgrounded with [`/detach`](/docs/tui#detach).
...
If you omit `url`, OpenCode looks up the current project's discovery record (written by `/detach` or a `SIGHUP` auto-detach) and connects to it automatically, inheriting its credentials.

#### Session resume

Which session `attach` connects you to depends on the flags you pass:

- `--session <id>` -- attach to that exact session.
- `--new` -- always start a new session, skipping resume entirely.
- `--continue` -- resume the project's most recently updated top-level session, same meaning as elsewhere in the CLI.
- No flag, run from a TTY -- OpenCode shows a picker of the project's top-level sessions, newest first. The session that was active when the server was backgrounded (from the discovery record) is preselected, so pressing Enter resumes exactly where [`/detach`](/docs/tui#detach) left off. "Create new session" is the last entry. Pressing `Esc` or `Ctrl-C` cancels -- no session is created and `attach` exits.
- No flag, run without a TTY (piped input, scripts) -- the picker can't be shown, so OpenCode auto-resolves in the same priority order the picker would preselect: the discovery record's session if it still exists, else the most recent top-level session, else a new session.

The same resolution applies whether `url` is given explicitly or discovered automatically.

Once attached, the [`/shutdown`](/docs/tui#shutdown) TUI command stops the server you're connected to.
...
| <nobr><code>{"--continue"}</code></nobr> | `-c`  | Continue the last session                                                  |
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--new"}</code></nobr>      |       | Always create a new session, skipping the resume picker                    |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
...
Stop a server that was backgrounded with [`/detach`](/docs/tui#detach) or a `SIGHUP` auto-detach, for the current project.

```bash
opencode stop
```

This reads the project's discovery record, sends `SIGTERM` to the server process, and removes the record.

#### Flags

| Flag                                  | Short | Description                                            |
| ------------------------------------- | ----- | ------------------------------------------------------ |
| <nobr><code>{"--force"}</code></nobr> | `-f`  | Force kill the server (`SIGKILL` instead of `SIGTERM`) |
