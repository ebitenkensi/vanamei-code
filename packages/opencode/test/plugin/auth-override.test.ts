...
describe("plugin.auth-override", () => {
  it.instance(
    "user plugin overrides built-in openai auth",
    () =>
      Effect.gen(function* () {
        ...
        const plugin = pathToFileURL(path.join(pluginDir, "custom-openai-auth.ts")).href
        ...
        const openai = methods[ProviderV2.ID.make("openai")]
        expect(openai).toBeDefined()
        expect(openai.length).toBe(1)
        expect(openai[0].label).toBe("Test Override Auth")
